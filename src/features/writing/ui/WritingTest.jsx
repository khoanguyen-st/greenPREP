...
const WritingTest = () => {
  ...
  useEffect(() => {
    ...
    const processedParts = sortedParts.map((part) => ({
      ...part,
      Content: part.Content.replace(/^Part \d+:\s*/, ""),
    }));

    setData({ ...data, Parts: processedParts });
    if (data) {
  ...
  return (
    <div className="max-w-3xl mx-auto relative min-h-screen pb-24">
      <div className="flex flex-col gap-5 w-full">
        <Text className="text-left font-bold xs:mx-2 md:mx-0 mt-12 text-[30px] md:text-[40px] w-fit">
          Writing Test
        </Text>
        <Text className="font-bold xs:mx-2 md:mx-0 mb-5 text-[28px] md:text-[35px] w-full">
          Question {currentPartIndex + 1} of {data.Parts.length}
        </Text>
      </div>

      <Text className="xs:mx-2 md:mx-0 block text-lg md:text-[20px] mb-3 mt-8 font-semibold">{currentPart.Content}</Text>
