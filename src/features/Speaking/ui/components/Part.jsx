  ...
  return (
    <div className="flex flex-row bg-white rounded-xl h-screen w-screen">
      <div className="lg:w-auto sm:w-1/2 pt-28 pl-28">
        <h1 className="text-4xl font-bold text-[#003087] mb-6">{data?.title || "Speaking Test"}</h1>
        <div className="mb-8">
          <h2 className="text-2xl font-bold mb-9">{data?.subtitle || "Part 1 of 3"}</h2>
          <p className="mb-20 text-2xl">
            <span className="font-bold">{data?.partTitle || "Part One"}</span> - {data?.description || "In this part, I am going to ask you three short questions about yourself and your interests. You will have 30 seconds to reply to each question."}
          </p>
          <p className="text-2xl">{data?.instruction || "Click next to begin the speaking test."}</p>
        </div>
      </div>
      <div className="lg:w-1/3 sm:w-1/2 mx-8 shadow-[10px_10px_4px_rgba(0,48,135,0.25)] my-6 min-h-3/4 rounded-3xl border-2 border-solid border-[#003087]">
        <span className="flex justify-center m-10 text-5xl font-normal">{data?.instructionHeader || "Instruction..."}</span>
        ...
        {useCountdownTimer(data?.timer || 20)}
      </div>
    </div>
  );
};
